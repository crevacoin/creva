...
uint256 hashGenesisBlock("0x000003d22a6cc627c3927330744bb3d1ddc222058918d37baeb87f5a6095bb0f"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CrevaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CrevaCoin Signed Message:\n";
...
{
    // CrevaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    if(nHeight < 1551) 		nSubsidy = 4000000 * COIN;
else if(nHeight < 6668217)	nSubsidy = 150 * COIN;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan =3 * 60 * 20; // CrevaCoin: 1 day
static const int64 nTargetSpacing = 3 * 60; // CrevaCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CrevaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, crevacoin - DarkGravity v2, written by Evan Duffield - evan@crevacoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CrevaCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // CrevaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Google Bans China’s Website Certificate Authority After Security Breach";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1427989295;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1631311;
        ...
        assert(block.hashMerkleRoot == uint256("0x937267cd6b42123df71986a696c28759fbb31d61961ed45d2e964b72b3acc119"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xd2, 0xb1, 0xda}; // CrevaCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CrevaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CrevaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CrevaCoinMiner(CWallet *pwallet)
{
    printf("CrevaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("crevacoin-miner");
    ...
    try { loop {
        ...
        printf("Running CrevaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CrevaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CrevaCoinMiner, pwallet));
}
